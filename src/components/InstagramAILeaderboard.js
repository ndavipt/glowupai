...
const InstagramAILeaderboard = () => {
  ...
  const fetchBaseData = useCallback(async () => {
    ...
    try {
      ...
      // Include locally approved accounts that aren't in the API response yet
      try {
        const localAccounts = JSON.parse(localStorage.getItem('approvedAccounts') || '[]');
        
        if (localAccounts.length > 0) {
          console.log(`Found ${localAccounts.length} locally approved accounts`);
          
          // Find accounts that aren't in the API response yet
          const accountsToAdd = localAccounts.filter(localAccount => 
            !accountsData.some(account => account.username === localAccount.username)
          );
          
          // Add these accounts to the data
          if (accountsToAdd.length > 0) {
            console.log(`Adding ${accountsToAdd.length} locally approved accounts to leaderboard`);
            
            accountsToAdd.forEach(localAccount => {
              // Create a placeholder account until real data arrives from API
              accountsData.push({
                id: localAccount.id || Date.now(),
                username: localAccount.username,
                // Other fields will be populated with profile data later
              });
            });
          }
        }
      } catch (err) {
        console.error('Error including locally approved accounts:', err);
      }
    ...
    <div className="leaderboard-container">
      ...
      <header className="leaderboard-header">
        ...
        <div className="header-buttons">
          <button className="nav-button" onClick={() => navigate('/submit')}>
            <img 
              src="/submit-icon.png" 
              alt="Submit Icon" 
